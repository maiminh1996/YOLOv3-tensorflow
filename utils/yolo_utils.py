...
# def get_training_data(annotation_path, data_path, input_shape, anchors, num_classes,load_previous=True, max_boxes=20, jitter=.3, hue=.1, sat=1.5, val=1.5, proc_img=True, random=False):
...
#                     image_data1 = np.array(new_image) / 255.
#
#                 # correct boxes
...
#                 # plt.show()
#                 image_data.append(np.array(image_data1))
...
#
#     y_true = preprocess_true_boxes(box_data, input_shape[0], anchors, num_classes)  # TODO
    ...
    with open(annotation_path) as f:
        ...
        for line in (GG):
            ...
            for i, box in enumerate(line[1:]):
                ...
                # boxes[i:i + 1, 0:2] = (boxes[i:i + 1, 0:2] * new_size / image_size).astype('int32')
                boxes[i:i+1, 2:4] = (boxes[i:i+1, 2:4]*new_size/image_size + (input_size-new_size)/2).astype('int32')
                # boxes[i:i + 1, 2:4] = (boxes[i:i + 1, 2:4] * new_size / image_size).astype('int32')
                # for case 1
